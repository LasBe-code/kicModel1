...
public class BoardDao {
	...
	public void refStepAdd(int ref, int refstep) {
		...
		PreparedStatement pstmt = null;
		String sql = 
			"update board set refstep = refstep + 1 where ref = ? and refstep > ?";
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, ref);
	...
	public int boardCount(String boardid) {
		...
		String sql = 
			"select count(*) from board where boardid = ?"; // 끝에 꼭 공백 한칸
	...
	public Board boardOne(int num) {
		...
		ResultSet rs = null;
		String sql = 
			"select * from board where num = ?"; // 끝에 꼭 공백 한칸
	...
	public int boardUpdate(Board board) {
		...
		String sql = 
			"update board set subject = ? , content = ? , file1 = ?"
				+" where num = ?";
	...
	public void readCountUp(int num) { // 글 확인하면 조회수 +1
		Connection con = JdbcConection.getConnection();
		PreparedStatement pstmt = null;
